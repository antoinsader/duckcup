...
import {
    analyze_messages, get_messages_multiple_chats, get_telegram_entities
} from "../../lib/backend/telegram";
import {
    is_date_in_filter_preset, is_row_in_specific_date_range,
...
export default function TelegramPage() {
    const { accounts_data, accounts_loading, accounts_error, refreshDatasets, datasets_data } = useUserContext();
    ...
    const [selected_senders_by_account, set_selected_senders_by_account] = useState({});
    // each account_id would have str of the date_filter_id
    const [selected_date_filter_by_account, set_selected_date_filter_by_account] = useState({});
    // each account_id would have array  of selected keyword filters
    const [selected_keyword_filters_by_account, set_selected_keyword_filters_by_account] = useState({});
    // each account_id would have a date value
    const [start_date_range_filter_by_account, set_start_date_range_filter_by_account] = useState({});
    const [end_date_range_filter_by_account, set_end_date_range_filter_by_account] = useState({});
    ...
    const [account_keywords_mapping_messages, set_account_keywords_mapping_messages] = useState({});
    const [keywords_loading_by_account, set_keywords_loading_by_account] = useState({});
    ...
    const [db_messages_loading_by_account, set_db_messages_loading_by_account] = useState({});
    const [db_messages_error_by_account, set_db_messages_error_by_account] = useState({});
    ...
    const sidebar_items = useMemo(() => {
        return telegram_accounts.map((tg_account, idx) => {
            ...
            const account_selected_senders = selected_senders_by_account[account_id] || {};

            const account_children = account_senders.filter(s => account_selected_senders[s.chat_id] === true ).map(sender => {
                
                const limit = limits_by_account_sender[account_id][sender.chat_id] ? `Limit: ${String(limits_by_account_sender[account_id][sender.chat_id])}` : null
    ...
    }, [telegram_accounts, senders_by_account, senders_loading_by_account, limits_by_account_sender, selected_senders_by_account]);
    ...
    const fetch_account_messages = useCallback(async (account_id, updated_account_selected_senders = null, updated_account_limits = null) => {
        ...
        try {
            ...
            const selected_senders = updated_account_selected_senders ? updated_account_selected_senders : selected_senders_by_account[account_id] || {};
            const account_limits = updated_account_limits ? updated_account_limits : limits_by_account_sender[account_id] || {};


            const active_sender_chat_ids = Object.keys(selected_senders).filter(chat_id => selected_senders[chat_id] === true).map(chat_id => String(chat_id));
            ...
            const [messages_response, analysis_response] = await Promise.all([req_1, req_2]);
            const messages = Array.isArray(messages_response) ? messages_response : [];
    ...
    const handle_sidebar_account_click = useCallback(async (account_id) => {
    ...
    const toggle_account_section_open = useCallback(async (account_id) => {
        if (!account_id) return;
    ...
    const sender_selected_function = useCallback((sender_row, account_id) => {
        const chat_id = sender_row?.chat_id;
        if (!chat_id || !account_id) return;



        const is_sender_active_prev = selected_senders_by_account?.[account_id]?.[chat_id] === true;
    ...
    const keyword_filter_select_function = useCallback((account_id, account_keyword) => {
        if (!account_id || !account_keyword) return;

        const is_keyword_active_prev = selected_keyword_filters_by_account?.[account_id]?.includes(account_keyword) === true;
        ...
        set_selected_keyword_filters_by_account(prev => {
            ...
            if (new_keyword_active) account_prev_keywords.push(account_keyword);
                ...
                const keyword_index = account_prev_keywords.findIndex(keyword => keyword === account_keyword);
                if (keyword_index >= 0) account_prev_keywords.splice(keyword_index, 1);
            }
            return {
                ...prev,
    ...
    const sender_limit_change_function = useCallback((sender_row, new_limit, account_id) => {
    ...
    const date_filter_select_function = useCallback((date_filter_id, account_id) => {
        if (!account_id) return;
        ...
        set_selected_date_filter_by_account(prev => ({
            ...prev,
            [account_id]: prev?.[account_id] === date_filter_id ? null : date_filter_id,
        }));


    }, []);

    const start_date_range_filter_select_function = useCallback((account_id, e) => {
    ...
    const get_filtered_messages = useCallback((
        ...
        if (account_selected_keyword_filter && account_selected_keyword_filter.length > 0) {
            const filtered_message_ids_set = new Set();
            account_selected_keyword_filter.forEach(k => {
                const k_msgs_ids = account_keywords_mapping_messages?.[account_id]?.[k] || [];
                k_msgs_ids.forEach(msg_id => filtered_message_ids_set.add(msg_id));
            })
            msgs = msgs.filter(msg => filtered_message_ids_set.has(String(msg.message_id)));
        }
        ...
        if (selected_start_date_filter || selected_end_date_filter) {
            msgs = msgs.filter(msg => {
                const msg_date = msg?.date;
                if (!msg_date) return true;
                return is_row_in_specific_date_range(msg_date, selected_start_date_filter, selected_end_date_filter);
            });
        }
    ...
    return (
        <div className={styles.telegram_page_root}>
            ...
            <ExtraSideBar
                ...
                default_open_item_ids={telegram_accounts && telegram_accounts.length > 0 ? [telegram_accounts[0].account_id] : []}
            />
            ...
            <div className={styles.accounts_cards_root}>
                {
                    telegram_accounts.map((account, account_idx) => {
                        ...
                        return (<AccountSection
                            ...
                            keywords_loading={keywords_loading_by_account[account_id] || false}



                            account_keywords={keywords_by_account[account_id] || []}
                            account_selected_keyword_filter={selected_keyword_filters_by_account?.[account_id] || null}
                            keywords_error={keywords_error_by_account?.[account_id]}
                            account_db_messages={db_messages_by_account[account_id] || []}
                            account_start_date_range_filter={start_date_range_filter_by_account[account_id] || null}
                            account_end_date_range_filter={end_date_range_filter_by_account[account_id] || null}
                            messages_loading={db_messages_loading_by_account[account_id] || false}
                            get_filtered_messages={get_filtered_messages}
                            account_keywords_mapping={account_keywords_mapping_messages?.[account_id] || {}}
                            ...
                            start_date_range_filter_select_function={start_date_range_filter_select_function}
                            end_date_range_filter_select_function={end_date_range_filter_select_function}

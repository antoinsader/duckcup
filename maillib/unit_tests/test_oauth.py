...
class TestOAuthState:
    def test_generate_returns_string_token(self, oauth_state):
        token = oauth_state.generate_oauth_token(user_id=1, provider_id=42)
        assert isinstance(token, str)
        assert len(token) > 0

    def test_generate_tokens_are_unique(self, oauth_state):
        token1 = oauth_state.generate_oauth_token(1, 42)
        token2 = oauth_state.generate_oauth_token(1, 42)
        assert token1 != token2

    def test_read_valid_state_returns_data(self, oauth_state):
        token = oauth_state.generate_oauth_token(user_id=5, provider_id=10)
    ...
    def test_read_expired_state_returns_none(self, expired_oauth_state):
        token = expired_oauth_state.generate_oauth_token(1, 2)
    ...
    def test_consume_removes_state(self, oauth_state):
        token = oauth_state.generate_oauth_token(1, 42)
    ...
    def test_validate_returns_state_data(self, oauth_state):
        token = oauth_state.generate_oauth_token(user_id=7, provider_id=3)
    ...
    def test_validate_consume_true_removes_state(self, oauth_state):
        token = oauth_state.generate_oauth_token(user_id=1, provider_id=2)
